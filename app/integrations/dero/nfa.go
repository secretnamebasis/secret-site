...
310 STORE("royalty", ` + royalty + `)
320 STORE("ownerCanUpdate", 1)
330 STORE("nameHdr", "` + nameHdr + `")
340 STORE("descrHdr", "` + descHdr + `")
...
20  STORE("owner", ADDRESS_RAW("` + addr + `"))
...
600 SEND_ASSET_TO_ADDRESS(ADDRESS_RAW("` + addr + `"), 1, SCID())
610 RETURN 0
999 RETURN 1
End Function
